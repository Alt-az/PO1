...
class rcstring::Cref {
  ...
  public:
	...
	rcstring::Cref& operator=(const Cref& ref) { return operator=((char)ref); }
};
...
inline int rcstring::atoi() {
	int a = 0;
	int i = 0;
	int sign = 1;
	while (isspace(data->s[i]))
		i++;
	if (data->s[i] == '-') {
		sign = -1;
		i++;
...
rcstring rcstring::left(int n) {
	check(n);
	rcstring k;
	rctext* newdata = new rctext(n + 1, k.data->s);
	delete k.data;
	k.data = newdata;
	k.data->size = (unsigned int)n;
	memcpy(k.data->s, data->s, k.data->size);
	k.data->s[k.data->size] = '\0';
	return k;
}
